...
#### Period Alias Candidate Generation (disabled)

BLS commonly detects at harmonic aliases -- 2x or 0.5x the true orbital period. In benchmark testing, 5 of 9 matched periods were aliases, causing ~38% planet radius error. Functions `_generate_alias_candidates()` and `_measure_phase_fold_depth()` exist in `transit.py` to generate alias candidates at P/2 and 2P, but are **currently disabled** -- the call is removed from `detect_transit()` pending a better disambiguation strategy.
