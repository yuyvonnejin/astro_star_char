...
def target():
    """Resolved target dict, shared across tests in this module."""
    t = get_target("82 G. Eridani")
    assert t is not None, "82 G. Eridani not in catalog"
    return resolve_target_ids(t)


def test_resolve_ids(target):
    """Step 1: Resolve Gaia DR3 and TIC IDs via SIMBAD."""
    logger.info("=" * 60)
    logger.info("Step 1: Resolving identifiers for 82 G. Eridani")
    logger.info("=" * 60)

    logger.info("Resolved Gaia DR3 ID: %s", target.get("gaia_dr3_id"))
    logger.info("Resolved TIC: %s", target.get("tic"))
    assert target.get("gaia_dr3_id") is not None
...
def test_pma(target):
    ...
    if gaia_id is None:
        pytest.skip("No Gaia ID resolved")
...
def test_full_report(target):
    ...
    assert report.get("stellar_properties", {}).get("status") == "ok"


def main():
    """Standalone runner: python tests/test_82_eridani_integration.py"""
    logger.info("Integration test: 82 G. Eridani (HD 20794)")
    logger.info("This test requires network access.")

    t = get_target("82 G. Eridani")
    resolved = resolve_target_ids(t)
    test_resolve_ids(resolved)
    test_known_planets()
    test_pma(resolved)
    test_full_report(resolved)
